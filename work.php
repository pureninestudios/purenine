<?php include ('header.php'); ?>

	<main id="content" class="work-backgrd" role="main">
		<div class="container">
			<p class="filter-selectors" data-container="my-projects1">
				<a href="#" class="active" data-filter="all">All</a>
				<a href="#" data-filter="category-design">Design</a>
				<a href="#" data-filter="category-mobile">Mobile</a>
				<a href="#" data-filter="category-development">Development</a>
			</p>
			<div class="filter-container row" id="my-projects1">
				<a href="project.html">
					<div class="filter-item grid-item col-xs-6 col-sm-6 col-md-4" data-id="1" data-type="category-design category-mobile">
						<div class="thumb">
							<div class="photo">
								<img src="https://s3-us-west-2.amazonaws.com/purenine/hp-madden.jpg" alt="" width="640" height="480">
								<div class="faux-bg" style="background-color:#422673"></div>
							</div>
							<div class="overlay-block">
								<div class="overlay-content">
									<p class="name">GOODGAME AGENCY</p>
									<h3>Madden Live Challenge</h3>
									<p class="tags">Art Direction, UX, UI, Visual Design</p>
								</div><!--/.overlay-content-->
							</div>
						</div>
					</div>
				</a>
				<a href="project.html">
					<div class="filter-item grid-item col-xs-6 col-sm-6 col-md-4" data-id="1" data-type="category-design category-mobile category-development">
						<div class="thumb">
							<div class="photo">
								<img src="https://s3-us-west-2.amazonaws.com/purenine/hp-pga.jpg" alt="" width="640" height="480">
								<div class="faux-bg" style="background-color:#021E36;"></div>
							</div>
							<div class="overlay-block">
								<div class="overlay-content">
									<p class="name">Omnigon</p>

									<h3>PGATour.com</h3>

									<p class="tags">UX, Visual Design, Mobile</p>
								</div><!--/.overlay-content-->
							</div>
						</div>
					</div>
				</a>
				<a href="project.html">
					<div class="filter-item grid-item col-xs-6 col-sm-6 col-md-4" data-id="1" data-type="category-design category-mobile">
						<div class="thumb">
							<div class="photo">
								<img src="https://s3-us-west-2.amazonaws.com/purenine/hp-proactiv.jpg">

								<div class="faux-bg" style="background-color:#54A1A9"></div>

							</div>
							<div class="overlay-block">
								<div class="overlay-content">

									<p class="name">Haven Agency</p>

									<h3>Proactiv.com</h3>

									<p class="tags">Art Direction, UX, Visual Design</p>

								</div><!--/.overlay-content-->
							</div>
						</div>
					</div>
				</a>
				<a href="project.html">
					<div class="filter-item grid-item col-xs-6 col-sm-6 col-md-4" data-id="1" data-type="category-design category-mobile">
						<div class="thumb">
							<div class="photo">
								<img src="https://s3-us-west-2.amazonaws.com/purenine/hp-guardian.jpg">

								<div class="faux-bg" style="background-color:#124074"></div>
							</div>
							<div class="overlay-block">
								<div class="overlay-content">

									<p class="name">Guardianlife</p>

									<h3>GuardianLife.com</h3>

									<p class="tags">Art Direction, Visual Design, Styleguide</p>

								</div><!--/.overlay-content-->
							</div>
						</div>
					</div>
				</a>
				<a href="project.html">
					<div class="filter-item grid-item col-xs-6 col-sm-6 col-md-4" data-id="1" data-type="category-design category-mobile">
						<div class="thumb">
							<div class="photo">
								<img src="https://s3-us-west-2.amazonaws.com/purenine/hp-am.jpg">

								<div class="faux-bg" style="background-color:#323232"></div>
							</div>
							<div class="overlay-block">
								<div class="overlay-content">

									<p class="name">Audiomack</p>

									<h3>Audiomack.com</h3>

									<p class="tags">Creative Direction, UX, UI, Visual Design, Mobile</p>

								</div><!--/.overlay-content-->
							</div>
						</div>
					</div>
				</a>
				<a href="project.html">
					<div class="filter-item grid-item col-xs-6 col-sm-6 col-md-4" data-id="1" data-type="category-design category-mobile category-development">
						<div class="thumb">
							<div class="photo">
								<img src="https://s3-us-west-2.amazonaws.com/purenine/hp-activision.jpg">

								<div class="faux-bg" style="background-color:#353B4B"></div>
							</div>
							<div class="overlay-block">
								<div class="overlay-content">

									<p class="name">Haven Agency</p>

									<h3>Shop Activision.com</h3>

									<p class="tags">Visual Design, UX, E-commerce</p>

								</div><!--/.overlay-content-->
							</div>
						</div>
					</div>
				</a>
				<a href="project.html">
					<div class="filter-item grid-item col-xs-6 col-sm-6 col-md-4" data-id="1" data-type="category-design category-mobile">
						<div class="thumb">
							<div class="photo">
								<img src="https://s3-us-west-2.amazonaws.com/purenine/hp-pennzoil.jpg">

								<div class="faux-bg" style="background-color:#303030"></div>
							</div>
							<div class="overlay-block">
								<div class="overlay-content">

									<p class="name">Haven Agency</p>

									<h3>Pennzoil Sweepstakes</h3>

									<p class="tags">Concept, UX, Visual Design, Social</p>

								</div><!--/.overlay-content-->
							</div>
						</div>
					</div>
				</a>
				<a href="project.html">
					<div class="filter-item grid-item col-xs-6 col-sm-6 col-md-4" data-id="1" data-type="category-design category-mobile category-development">
						<div class="thumb">
							<div class="photo">
								<img src="https://s3-us-west-2.amazonaws.com/purenine/hp-1d.jpg">

								<div class="faux-bg" style="background-color:#8F5EA0"></div>
							</div>
							<div class="overlay-block">
								<div class="overlay-content">

									<p class="name">Livenation</p>

									<h3>One Direction Tour</h3>

									<p class="tags">UX, UI, Visual Design</p>

								</div><!--/.overlay-content-->
							</div>
						</div>
					</div>
				</a>
				<a href="project.html">
					<div class="filter-item grid-item col-xs-6 col-sm-6 col-md-4" data-id="1" data-type="category-design category-mobile">
						<div class="thumb">
							<div class="photo">
								<img src="https://s3-us-west-2.amazonaws.com/purenine/hp-highlights.jpg">

								<div class="faux-bg" style="background-color:#415266"></div>
							</div>
							<div class="overlay-block">
								<div class="overlay-content">

									<p class="name">Highlights</p>

									<h3>Highlights.com</h3>

									<p class="tags">Pitch, Visual Design, UX, UI, Mobile</p>

								</div><!--/.overlay-content-->
							</div>
						</div>
					</div>
				</a>
				<a href="project.html">
					<div class="filter-item grid-item col-xs-6 col-sm-6 col-md-4" data-id="1" data-type="category-design category-mobile">
						<div class="thumb">
							<div class="photo">
								<img src="https://s3-us-west-2.amazonaws.com/purenine/hp-bio.jpg">

								<div class="faux-bg" style="background-color:#024D77"></div>
							</div>
							<div class="overlay-block">
								<div class="overlay-content">

									<p class="name">A+E Networks</p>

									<h3>Biography.com</h3>

									<p class="tags">Creative Direction, UX, UI, Visual Design</p>

								</div><!--/.overlay-content-->
							</div>
						</div>
					</div>
				</a>
				<a href="project.html">
					<div class="filter-item grid-item col-xs-6 col-sm-6 col-md-4" data-id="1" data-type="category-design category-mobile">
						<div class="thumb">
							<div class="photo">
								<img src="https://s3-us-west-2.amazonaws.com/purenine/hp-indy.jpg">

								<div class="faux-bg" style="background-color:#CA1B18"></div>
							</div>
							<div class="overlay-block">
								<div class="overlay-content">

									<p class="name">Omnigon</p>

									<h3>Verizon IndyCar</h3>

									<p class="tags">Visual Design, Mobile, Tablet</p>

								</div><!--/.overlay-content-->
							</div>
						</div>
					</div>
				</a>
				<a href="project.html">
					<div class="filter-item grid-item col-xs-6 col-sm-6 col-md-4" data-id="1" data-type="category-design category-mobile">
						<div class="thumb">
							<div class="photo">
								<img src="https://s3-us-west-2.amazonaws.com/purenine/hp-toyota.jpg">

								<div class="faux-bg" style="background-color:#666666"></div>
							</div>
							<div class="overlay-block">
								<div class="overlay-content">

									<p class="name">People.com</p>

									<h3>Toyota Avalon</h3>

									<p class="tags">Advertising, UX, UI, Visual Design, Tablet</p>

								</div><!--/.overlay-content-->
							</div>
						</div>
					</div>
				</a>
				<a href="project.html">
					<div class="filter-item grid-item col-xs-6 col-sm-6 col-md-4" data-id="1" data-type="category-design category-mobile">
						<div class="thumb">
							<div class="photo">
								<img src="https://s3-us-west-2.amazonaws.com/purenine/hp-jt.jpg">

								<div class="faux-bg" style="background-color:#232323"></div>
							</div>
							<div class="overlay-block">
								<div class="overlay-content">

									<p class="name">Livenation</p>

									<h3>Legends of Summer Tour </h3>

									<p class="tags">Creative Direction, UX, UI, Visual Design</p>

								</div><!--/.overlay-content-->
							</div>
						</div>
					</div>
				</a>
				<a href="project.html">
					<div class="filter-item grid-item col-xs-6 col-sm-6 col-md-4" data-id="1" data-type="category-design category-mobile">
						<div class="thumb">
							<div class="photo">
								<img src="https://s3-us-west-2.amazonaws.com/purenine/hp-mk.jpg">

								<div class="faux-bg" style="background-color:#301011"></div>
							</div>
							<div class="overlay-block">
								<div class="overlay-content">

									<p class="name">Haven Agency</p>

									<h3>Mortal Kombat</h3>

									<p class="tags">Pitch, Concept, UX, UI, Visual Design</p>

								</div><!--/.overlay-content-->
							</div>
						</div>
					</div>
				</a>
				<a href="project.html">
					<div class="filter-item grid-item col-xs-6 col-sm-6 col-md-4" data-id="1" data-type="category-design category-mobile">
						<div class="thumb">
							<div class="photo">
								<img src="https://s3-us-west-2.amazonaws.com/purenine/hp-thq.jpg">

								<div class="faux-bg" style="background-color:#442956"></div>
							</div>
							<div class="overlay-block">
								<div class="overlay-content">

									<p class="name">Haven Agency</p>

									<h3>THQ.com</h3>

									<p class="tags">Pitch, Concept, Visual Design</p>

								</div><!--/.overlay-content-->
							</div>
						</div>
					</div>
				</a>


			</div>
			<div class="clear"></div>
		</div>
	</main>

<?php include ('footer.php'); ?>