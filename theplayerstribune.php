<?php include('header.php'); ?>

	<div class="scroll-line-track"><span class="scroll-line"></span></div>
		<!-- Page Content -->
		<div id="page-content">


		<!-- Hero -->
        <div id="hero" class="hidden hero-large">
			<div id="hero-styles" class=" parallax-hero opacity-hero">

			<div class="hero-image" style="">
            	<div class="overlay" style="background-color:rgba(255, 255, 255, 1.0)">
                    <!-- Slide Caption -->
                    <div class="clapat-caption animated ">
                        <div class="caption-content center-center">
                            <h1 class="big-title">The Players Tribune</h1>
							<p style="padding-bottom: 10px;">Designed an app that provides athletes with a platform to connect directly with their fans.</p>

							<ul class="hero-info">
								<li>
									Client: <span>The Players Tribune</span>
								</li>
								<li>
									Agency: <span>HYFN</span>
								</li>
								<li>
									Services: <span>Visual Design, UX, UI, App, Pitch</span>
								</li>
							</ul>
						</div>
                    </div>
                    <!--/Slide Caption -->
                </div>
            </div>
						<div class="hero-scroll-icons">
            	<div class="icon-scroll"></div>
            	<div class="icon-scroll-line"></div>
            </div>
						</div>
		</div>
        <!--/Hero -->

					<!-- Main -->
			<div id="main">
				<div id="main-content" class="hidden">

					<div class="single-project-inner">

						<img class="single-project-image" src="imgs/portfolio-tp-1.jpg" alt="" />

						<img class="single-project-image" src="imgs/portfolio-tp-2.jpg" alt="" />

					</div>

					
				</div>
			</div>
			<!--/Main -->

<?php include('footer.php'); ?>
