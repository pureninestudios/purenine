<?php include('header.php'); ?>
		<!-- Page Content -->
		<div id="page-content">


		<!-- Hero -->
        <div id="hero" class="hidden hero-large">
			<div id="hero-styles" class=" parallax-hero opacity-hero">

			<div class="hero-image" style="">
            	<div class="overlay" style="background-color:rgba(255, 255, 255, 1.0)">
                    <!-- Slide Caption -->
                    <div class="clapat-caption animated ">
                        <div class="caption-content center-center">
                            <h1 class="big-title">Pennzoil<br>Sweepstakes</h1>
							<p style="padding-bottom: 10px;">Conceptualized ideas and designed a sweepstakes website for Pennzoil and Livenation.</p>

							<ul class="hero-info">
								<li>
									Client: <span>Livenation</span>
								</li>

								<li>
									Agency: <span>Haven Agency</span>
								</li>

								<li>
									Services: <span>Art Direction, Visual Design, UX, UI</span>
								</li>
							</ul>
						</div>
                    </div>
                    <!--/Slide Caption -->
                </div>
            </div>
						<div class="hero-scroll-icons">
            	<div class="icon-scroll"></div>
            	<div class="icon-scroll-line"></div>
            </div>
						</div>
		</div>
        <!--/Hero -->

					<!-- Main -->
			<div id="main">
				<div id="main-content" class="hidden">

					<div class="single-project-inner">

						<img class="single-project-image" src="imgs/portfolio-pennzoil-1.jpg" alt="" />


					</div>

					<div class="vc_row wpb_row vc_row-fluid text-align-center vc_custom_1460234445834"><div class="container"><div class="wpb_column vc_column_container vc_col-sm-12"><div class="vc_column-inner "><div class="wpb_wrapper"><h2>Available for new projects<br> & freelance gigs.</h2><div class="vc_empty_space"  style="height: 20px" ><span class="vc_empty_space_inner"></span></div>
<a class="clapat-button animation-link" href="../contact/index.html" target="_self">Drop us a line</a></div></div></div></div></div>
				</div>
			</div>
			<!--/Main -->

<?php include('footer.php'); ?>
